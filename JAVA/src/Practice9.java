...
public class Practice9 
{
	...
	static void compare(String refDNA, String myDNA, int k, int threshold,long time)
	{
		try
		{
			BufferedWriter fw = new BufferedWriter(new FileWriter("./"+k+"_information.txt",true));	// 일치도와 불일치개수, 복원시간이 입력될 파일객체
	...
	static void compare(String refDNA, String myDNA, int k)
	{
		try
		{
			BufferedWriter fw = new BufferedWriter(new FileWriter("./information.txt",true));	// 일치도와 불일치개수, 복원시간이 입력될 파일객체
	...
	public static void main(String[] args)
	{
		try
		{
			...
			String refDNA_path = ".//"+"referenceDNA.txt";	//referenceDNA 읽어오는 객체
			...
			{
				// shortRead를 읽어오는 객체
				BufferedReader reader = new BufferedReader(new FileReader("./shortRead_"+i+".txt"));	
			...
			{
				BufferedWriter fw = new BufferedWriter(new FileWriter("./"+k+"_"+threshold+"_myDNA.txt"));	// myDNA가 입력되는 객체
			...
			BufferedReader fin = new BufferedReader(new FileReader("./"+k+"_"+14+"_myDNA.txt"));
