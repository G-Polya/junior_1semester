			...
			try
			{
				...
				case 2:
					...
					while(true)
					{
            			...
            			for(int i = 0; i<nations.size();i++)	// nations의 모든 원소에 대하여
            			{
            				String temp_country = nations.elementAt(i).getCountry();	
            				if(temp_country.equals(key))		// 해당 인덱스의 country가 key와 같다.
            				{
            					if(answer.equals(nations.elementAt(i).getCapital())) // 그 key의 capital이  답으로 입력한 값이다.
        						{
